...
class CenterPointUncertaintyLoss(nn.Module):
    def __init__(self, args):
        super(CenterPointUncertaintyLoss, self).__init__()
    ...
    def get_unc_by_boxes(self,output_dict,target_dict,suffix="_single"):
        box_preds = output_dict['bbox_preds{}'.format(suffix)].permute(
            0, 2, 3, 1).contiguous()  # [B, H, W, C]
        cls_preds = clip_sigmoid(output_dict['cls_preds{}'.format(suffix)])
        unc_preds = output_dict['unc_preds{}'.format(suffix)].permute(0, 2, 3, 1).contiguous()  # [B, H, W, C]
        bbox_center = target_dict['object_bbx_center{}'.format(
            suffix)].cpu().numpy()
        bbox_mask = target_dict['object_bbx_mask{}'.format(
            suffix)].cpu().numpy()
        ...
        max_gt = int(max(bbox_mask.sum(axis=1)))
                # [B, max_anchor_num, 7]
        gt_boxes3d = np.zeros(
            (batch_size, max_gt, bbox_center[0].shape[-1]), dtype=np.float32)
        for k in range(batch_size):
            gt_boxes3d[k, :int(bbox_mask[k].sum()),
                       :] = bbox_center[k, :int(bbox_mask[k].sum()), :]
        gt_boxes3d = torch.from_numpy(gt_boxes3d).to(box_preds.device)

        targets_dict = self.assign_targets(
            # [B, max_anchor_num, 7 + C ]      heatmap [2,1,h,w]  anno_boxes [2,100,8] inds [2, 100]
            gt_boxes=gt_boxes3d
        )
        cls_gt = targets_dict['heatmaps']
        
        box_gt = (targets_dict['anno_boxes'],
                  targets_dict['inds'], targets_dict['masks'])
        target_box, inds, masks = box_gt
        pred = box_preds
        ind = inds
        pred_unc = unc_preds.view(unc_preds.size(0), -1, unc_preds.size(3))     # [n, h*w, 2 ]
        pred_unc = self._gather_feat(pred_unc, ind)
        return pred_unc
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        # unc_loss,scatter_data = self.get_unc_layer_loss(box_preds,box_gt,unc_preds)
        unc_loss= self.get_unc_layer_loss(box_preds,box_gt,unc_preds)


        rpn_loss = cls_loss + 1*box_loss + 0.3*unc_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer=None, suffix=""):
        ...
        print("[epoch %d][%d/%d]%s, || Loss: %.4f || Conf Loss: %.4f"
              ...
              "|| Unc Loss:%.4f" % (
                  ...
                  total_loss, cls_loss, reg_loss,unc_loss))
    ...
    def get_unc_layer_loss(self,bbox_preds,bbox_gt,pred_unc,reg_weights=None):
        ...
        pred_unc = pred_unc.view(pred_unc.size(0), -1, pred_unc.size(3))     # [n, h*w, 2 ]
        pred = self._gather_feat(pred, ind)
        pred_unc = self._gather_feat(pred_unc, ind)
        mask = masks.unsqueeze(2).expand_as(
            target_box).float()  # 把 mask 的维度进行扩展
        isnotnan = (~torch.isnan(target_box)).float()
        mask *= isnotnan
        
        pred=pred*mask
        target_box=target_box*mask
        pred_unc=pred_unc*mask[...,:1]

        # unc_loss,scatter_data = self.loss_unc(pred,
        #                              target_box,
        #                              pred_unc,
        #                              reg_weights)
        unc_loss = self.loss_unc(pred,
                                     target_box,
                                     pred_unc,
                                     reg_weights)
        # unc_loss = unc_loss * self.cls_weight
        # print('unc_loss', unc_loss.shape)
        
        unc_loss = unc_loss.sum() / max(num, 1)
        return unc_loss
    ...
    def _gather_feat(self, feat, ind, mask=None):
        ...
        ind = ind.unsqueeze(2).expand(
            ind.size(0), ind.size(1), dim)   # 把 ind 和 dim 拼接在一起
    ...
    def assign_targets(self, gt_boxes):
        ...
        if gt_boxes.shape[-1] == 8:
            # gt_box [2,14,8] batch_size * bbox_num * 8
    ...
    def get_targets_single(self, gt_bbox_3d, gt_labels_3d=None):
        ...
        for batch in range(batch_size):
            ...
            for k in range(num_objs):
                # 计算x的heatmap坐标
                coor_x = (task_boxes[k][0] - pc_range[0]) / \
                    voxel_size[0] / self.target_cfg['out_size_factor']
                coor_y = (task_boxes[k][1] - pc_range[1]) / \
                    voxel_size[1] / self.target_cfg['out_size_factor']
                coor_z = (task_boxes[k][2] - pc_range[2]) / \
                    voxel_size[2] / self.target_cfg['out_size_factor']
                h = task_boxes[k][3] / voxel_size[0] / \
                    self.target_cfg['out_size_factor']
                w = task_boxes[k][4] / voxel_size[1] / \
                    self.target_cfg['out_size_factor']
                l = task_boxes[k][5] / voxel_size[2] / \
                    self.target_cfg['out_size_factor']
                rot = task_boxes[k][6]
                ...
                if h > 0 and w > 0:
                    ...
                    # box_dim = box_dim.log()
                    box_dim = torch.cat(
                        [h.unsqueeze(0), w.unsqueeze(0), l.unsqueeze(0)], dim=0)
