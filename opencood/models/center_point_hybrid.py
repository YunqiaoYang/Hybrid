...
class CenterPointHybrid(nn.Module):
    def __init__(self, args):
        super(CenterPointHybrid, self).__init__()
        print('CenterPointHybridModel')
        # PIllar VFE
        ...
        self.reg_head = nn.Conv2d(input_channels, 8 * args['anchor_number'],
                                  kernel_size=1)
        self.unc_head = nn.Conv2d(input_channels, 2 * args['anchor_number'],
                                  kernel_size=1)                         
    ...
    def init_weight(self):
        ...
        nn.init.constant_(self.reg_head.bias, -np.log((1 - pi) / pi) )
    ...
    def forward(self, data_dict):
        ...
        unc = self.unc_head(spatial_features_2d)
        #print("bbox.shape", bbox.shape)


        # 把bbox 的第二维度变成7 
        _, bbox_temp = self.generate_predicted_boxes(cls, bbox)
        #print("bbox_temp.shape", bbox_temp.shape)

        # print(bbox.equal(bbox_temp))
        output_dict = {'cls_preds': cls,
                       'reg_preds': bbox_temp,
                       'bbox_preds': bbox,
                       'unc_preds':unc}    # 计算loss的时候使用 'bbox', 在生成output的时候 'rm'
