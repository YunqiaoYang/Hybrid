...
class UncertaintyVoxelPostprocessordiff(VoxelPostprocessor):
    def __init__(self, anchor_params, train):
        super(UncertaintyVoxelPostprocessordiff, self).__init__(anchor_params, train)
    ...
    def post_process_stage1(self, stage1_output_dict, anchor_box):
        ...
        if 'dir_preds' in stage1_output_dict and len(boxes3d) != 0:
            ...
            ) # ÈôêÂà∂Âú?0Âà∞pi‰πãÈó¥
            boxes3d[..., 6] = dir_rot + dir_offset + period * dir_labels.to(boxes3d.dtype) # ËΩ?Âå?0.25piÂà?2.5pi
    ...
    def post_process(self, data_dict, output_dict, return_uncertainty=True,return_points=False):
        """
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            preds=output_dict[cav_id]['reg_preds'].permute(0, 2, 3, 1).contiguous().view(1, -1, 7)
            # print(preds.shape)
            
            gtbox=data_dict[cav_id]['label_dict']['targets']
            
            gtbox=gtbox.view(1, -1, 7)
            # with open('/GPFS/data/yunqiaoyang-1/Hybrid_opv2v_unc_diffxy/gtbox.yaml', "w", encoding="utf-8") as file:
            #     yaml.dump(gtbox.cpu().numpy().tolist(),file)
            # input()
            # print(gtbox.shape)
            # uncertainty map
            ...
            diffs=preds-gtbox
            # preds=torch.masked_select(gtbox[0],
            #                               mask_reg[0]).view(-1, 7)
            # gts=torch.masked_select(gtbox[0],
            #                               mask_reg[0]).view(-1, 7)

            diffs=torch.masked_select(diffs[0],mask_reg[0]).view(-1, 7)
            ...
            if 'dir_preds' in output_dict[cav_id].keys() and len(boxes3d) != 0:
                ...
                boxes3d[..., 6] = dir_rot + dir_offset + period * dir_labels.to(dir_cls_preds.dtype) # ËΩ?Âå?0.25piÂà?2.5pi
                boxes3d[..., 6] = limit_period(boxes3d[..., 6], 0.5, 2 * np.pi) # limit to [-pi, pi]
            ...
            if len(boxes3d) != 0:
                ...
                # for cav_id!=ego,add box noise
                if(cav_id!='ego' and self.params ['noise_setting']['pred_box_noise']):
                    
                    # print(projected_boxes3d)
                    projected_boxes3d=add_noise_box(projected_boxes3d,self.params ['noise_setting'])
                    # print('pred box noise')
                    # print(projected_boxes3d)
                # convert 3d bbx to 2d, (N,4)
                projected_boxes2d = \
                ...
                if cav_id!='ego':
                    tem_points_num=projected_boxes3d.shape[0]*8
            
            points_num+=tem_points_num
            
                

        # print('pred_3d_list')
        # print(len(pred_box3d_list))
        # print(pred_box3d_list[0].shape)
        # print(pred_box3d_list[1].shape)
        ...
        diffs_list=torch.vstack(diffs_list)
        ...
        diffs_list=diffs_list[keep_index]
        ...
        if return_uncertainty:
            if return_points:
                return pred_box3d_tensor, scores, uncertainty, points_num
            return pred_box3d_tensor, scores, uncertainty,diffs_list
        
        if return_points:
            return pred_box3d_tensor, scores, points_num
