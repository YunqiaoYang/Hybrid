...
class HybridVoxelPostprocessor(BasePostprocessor):
    def __init__(self, anchor_params, train):
        
        super(HybridVoxelPostprocessor, self).__init__(anchor_params, train)
    ...
    def post_process(self, data_dict, output_dict):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            # unc
            unc = output_dict[cav_id]['unc_preds']
            unc = torch.exp(unc.permute(0, 2, 3, 1))
            uncx= unc[:, :, :, 0]
            uncy= unc[:, :, :, 1]

            
            uncx= uncx.reshape(1, -1)
            uncy= uncy.reshape(1, -1)
            ...
            uncx = torch.masked_select(uncx[0], mask[0])
            uncy = torch.masked_select(uncy[0], mask[0])
            ...
            if len(boxes3d) != 0:
                ...
                # (N, 7)
                boxes2d_score = \
                    torch.cat((projected_boxes2d, scores.unsqueeze(1),uncx.unsqueeze(1),uncy.unsqueeze(1)), dim=1)
        ...
        # scores
        scores = pred_box2d_list[:, -3]
        uncx= pred_box2d_list[:, -2]
        uncy= pred_box2d_list[:, -1]
        # predicted 3d bbx
        ...
        uncx = uncx[keep_index]
        uncy = uncy[keep_index]
        ...
        assert uncx.shape[0] == pred_box3d_tensor.shape[0]
        assert uncy.shape[0] == pred_box3d_tensor.shape[0]
        uncertainty = torch.stack((uncx, uncy), dim=1)
