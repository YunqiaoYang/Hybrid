...
def visualize(infer_result, pcd1,pcd2, color1,color2,pc_range, save_path, method='3d', left_hand=False):
    """
    ...
    pcd_np1 = pcd1.cpu().numpy()
    pcd_np2 = pcd2.cpu().numpy()
    ...
    if method == 'bev':
        ...
        canvas_xy1, valid_mask1 = canvas.get_canvas_coords(pcd_np1) # Get Canvas Coords
        canvas.draw_canvas_points(canvas_xy1[valid_mask1],radius=1,colors=color1) # Only draw valid points
        canvas_xy2, valid_mask2 = canvas.get_canvas_coords(pcd_np2) # Get Canvas Coords
        canvas.draw_canvas_points(canvas_xy2[valid_mask2],radius=1,colors=color2) # Only draw valid points
