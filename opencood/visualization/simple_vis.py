...
def visualize(infer_result, pcd, pc_range, save_path, method='3d', left_hand=False):
        ...
        if pred_box_tensor is not None:
            ...
            pred_name = ['c'] * pred_box_np.shape[0]

            score = infer_result.get("pred_score", None)
            #print(score.shape)
            if score is not None:
                score_np = score
                pred_name = [f'c:{score_np[i]:.2f}' for i in range(score_np.shape[0])]

            # unc_x=infer_result.get("unc_x", None)
            # unc_y=infer_result.get("unc_y", None)
            # if (unc_x is not None) and(unc_y is not None):
            #     unc_x_np=unc_x.cpu().numpy()
            #     unc_y_np=unc_y.cpu().numpy()
            #     unc_np=unc_x_np+unc_y_np
            #     pred_name = [f'c:{score_np[i]:.2f},u:{unc_np[i]:.2f}' for i in range(score_np.shape[0])]
        ...
        if method == 'bev':
            ...
            canvas.draw_canvas_points(canvas_xy[valid_mask],radius=1,colors=(142, 207, 201)) # Only draw valid points
            if gt_box_tensor is not None:
